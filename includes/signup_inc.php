  ...
  { 
    ...
    if (!preg_match("/^[a-zA-Z]*$/", $first) || !preg_match("/^[a-zA-Z]*$/", $last)) {
      ...
      if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $sql = "SELECT * FROM tbl_benutzer WHERE user_uid='$uid'";
        ...
        if ($resultCheck > 0) {
          ...
          //Insert User in DB
          $sql = 'INSERT INTO tbl_benutzer (user_titel, user_position, user_first, user_last, user_firm, user_email, user_telefon, user_uid, user_pic, user_pwd, user_rang) VALUES ("' . mysql_real_escape_string($titel) . '", "' . mysql_real_escape_string($position) . '", "' . mysql_real_escape_string($first) . '", "' . mysql_real_escape_string($last) . '", "' . mysql_real_escape_string($firm) . '", "' . mysql_real_escape_string($email) . '", "' . mysql_real_escape_string($telefon) . '", "' . mysql_real_escape_string($uid) . '", "' . mysql_real_escape_string($defaultpic) . '", "' . mysql_real_escape_string($hashedPwd) . '", "' . mysql_real_escape_string($rang) . '");';
          mysqli_query($db, $sql);
